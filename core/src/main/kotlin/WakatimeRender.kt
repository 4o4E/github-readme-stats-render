...
object WakatimeRender {
    ...
    suspend fun renderLang(user: String, range: FetchRange, allLang: Boolean, theme: Theme): ByteArray {
        ...
        val titleLine = TextLine.make("$user's wakatime stats in ${range.display}", titleFont)
//        val barWidth = titleLine.width - maxLangName - 40 - maxLangText
        val width = max(titleLine.width.toInt(), maxLangName + barWidth.toInt() + barPadding.toInt() * 2 + maxLangText) + margin.toInt() * 2
    ...
    suspend fun renderEditor(user: String, range: FetchRange, theme: Theme): ByteArray {
        val stats = fetchUserStats(user, range)
        val langs = stats["editors"]!!.jsonArray.map {
            Item(it as JsonObject)
        }

        val maxLangName = langs.maxOf { it.nameWidth }.toInt()
        val maxLangText = langs.maxOf { it.textWidth }.toInt()
        val titleLine = TextLine.make("$user's editor usage stats in ${range.display}", titleFont)
//        val barWidth = titleLine.width - maxLangName - 40 - maxLangText
        val width = max(titleLine.width.toInt(), maxLangName + barWidth.toInt() + barPadding.toInt() * 2 + maxLangText) + margin.toInt() * 2
        val height = langs.size * spacing.toInt() + margin.toInt() * 2 + titleFont.metrics.run { descent - ascent }.toInt()
    ...
    data class Item(
        ...
        val nameLine = TextLine.make(name, langFont)
    ...
    suspend fun fetchUserStats(user: String, range: FetchRange): JsonObject {
        ...
        if (response.status == HttpStatusCode.NotFound) throw Exception("unknown wakatime user, please register at wakatime.com")
